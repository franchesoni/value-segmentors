...
def main():
    ...
    model = EfficientViTSegL2(
        use_norm_params=False,
        pretrained=False,
    )
    model = load_from_ckpt(model, args.weight_url)
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    model = model.to(device)
    print('loaded model from {}'.format(args.weight_url))
    ...
    with torch.inference_mode():
        with tqdm(
            ...
            for feed_dict in data_loader:
                images, mask = feed_dict["data"].to(device), feed_dict["label"].to(device)
                # compute output
                output = model(images)
                ...
                t.set_postfix(
                    {
                        "mIOU": torch.nanmean((interaction.sum / union.sum).cpu()).item() * 100,
